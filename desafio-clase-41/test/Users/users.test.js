...
export default () => {
    describe('Test de Users', () => {    
        ...
        it('Debe loguear correctamente al usuario y DEVOLVER UNA COOKIE', async function() {
            ...
            const result = await requester.post('/api/session/login').send({email: mockUser.email, password: mockUser.password})
        ...
        it('Debe eliminar al usuario registrado en la base de datos', async function () {
            // await requester.post('/api/session/login').send({email: mockUser.email, password: mockUser.password})
            const deletedMsg = await requester.delete(`/api/session/${idRegistered}`).set('Cookie', [`${cookie.name}=${cookie.value}`])
            const userDeleted = await requester.get(`/api/session/${idRegistered}`).set('Cookie', [`${cookie.name}=${cookie.value}`])
