...
export default () => {
    describe('Test de Carritos', () => {
        ...
        before( async function() {
            ...
            // logueo el usuario
            let loggedUser = await requester.post('/api/session/login').send({email: mockUser.email, password: mockUser.password})
            ...
            await requester.put(`/api/session/users/premium/${idRegisteredUser}`)
            ...
            loggedUser = await requester.post('/api/session/login').send({email: mockUser.email, password: mockUser.password})
            ...
            let adminLoggedUser = await requester.post('/api/session/login').send({email: env_parameters_obj.admin.adminEmail, password: env_parameters_obj.admin.adminPassword})
        ...
        after( async function() {
            ...
            await requester.delete(`/api/session/${idRegisteredUser}`)
                .set('Cookie', [`${cookieNewUser.name}=${cookieNewUser.value}`])
        })
    })
}